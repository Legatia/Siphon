...
import { BattleStatus, PROTOCOL_CONSTANTS } from "@siphon/core";
...
export default function BattleLivePage() {
  ...
  const handleSubmitResponse = async () => {
    if (!pendingRound || !myShardId || !response.trim()) return;
    ...
    try {
      const res = await fetch(`/api/battles/${battleId}`, {
        ...
        body: JSON.stringify({
          round: pendingRound.roundNumber,
          shardId: myShardId,
  ...
  return (
    <div className="space-y-6">
      ...
      {!isCompleted && isParticipant && pendingRound && (
        <Card className="border-siphon-teal/20">
          <CardHeader>
            <CardTitle className="text-base">
              Round {pendingRound.roundNumber} - Your Response
            </CardTitle>
          </CardHeader>
