  ...
  try {
    ...
    await dbRun(
      `INSERT INTO stake_payments (id, user_id, amount, tx_hash, confirmed, tier, created_at, expires_at)
       VALUES (?, ?, ?, ?, 1, ?, ?, NULL)`,
      crypto.randomUUID(), userId, amount, txHash, tier, now
    );
    ...
    if (existing) {
      await dbRun(
        `UPDATE subscriptions
         ...
         WHERE user_id = ?`,
        tier, amount, txHash, hostingType, now, userId
      );
    } else {
      await dbRun(
        `INSERT INTO subscriptions (id, user_id, tier, stake_amount, stake_tx_hash, hosting_type, message_count, last_message_reset, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, 0, 0, ?, ?)`,
        crypto.randomUUID(), userId, tier, amount, txHash, hostingType, now, now
      );
    }
